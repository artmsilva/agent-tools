...
| `pi-extensions` | [`packages/pi-extensions`](./packages/pi-extensions) | Grab-bag of small extensions: `dcg-guard` (destructive-command blocking), `worktree` (isolated worktrees with node_modules pre-linked), `open-zed`, `open-hunk`, `sanitize-error-results` (path install only, not on npm) |
