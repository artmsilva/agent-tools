...
| `pi-agent-teams` | [`packages/pi-agent-teams`](./packages/pi-agent-teams) | Continuous-conversation agent teams: shared tasks and mailboxes, worktree isolation, visible Herdr teammates with automatic headless RPC fallback |
...
| `pi-duet` | [`packages/pi-duet`](./packages/pi-duet) | Side conversations: `alt+u` or `/duet` asks a cheap model for a context-free second opinion; `/btw` asks the active model about the current session without tools or transcript writes |
| `pi-extensions` | [`packages/pi-extensions`](./packages/pi-extensions) | Grab-bag of small extensions: `dcg-guard` (destructive-command blocking), `worktree` (isolated worktrees with copy-on-write node_modules), `open-zed`, `open-hunk`, `sanitize-error-results` (path install only, not on npm) |
