...
/**
 * Recursively delete the given teamDir after safely removing clean git worktrees.
 *
 * Steps:
 * 1. Remove clean git worktrees via `cleanupWorktrees`; preserve local branches
...
export async function cleanupTeamDir(
	...
	// 1. Close persisted Herdr panes before deleting their recovery metadata.
	try {
		await new HerdrClient().cleanupTeam(teamDirAbs);
	} catch (err: unknown) {
		const message = err instanceof Error ? err.message : String(err);
		warnings.push(`Herdr cleanup failed; team directory preserved: ${message}`);
		return {
			teamDir: teamDirAbs,
			worktreeResult: {
				removedWorktrees: [],
	...
	// 2. Clean up safe git worktrees before deleting the directory. Branches are recovery artifacts.
	...
	// Never delete the containing team directory while a dirty or unverifiable
	// worktree is preserved inside it. That directory is the recovery artifact.
	if (worktreeResult.preservedWorktrees.length > 0) {
		warnings.push(
			`Team directory preserved because ${worktreeResult.preservedWorktrees.length} worktree(s) need recovery.`,
		);
		return { teamDir: teamDirAbs, worktreeResult, preserved: true, warnings };
	}

	await fs.promises.rm(teamDirAbs, { recursive: true, force: true });
	return { teamDir: teamDirAbs, worktreeResult, preserved: false, warnings };
}
...
export async function gcStaleTeamDirs(opts: {
	...
	for (const teamId of teamEntries) {
		...
		try {
			...
			if (result.preserved) skipped.push({ teamId, reason: "cleanup preserved recovery state" });
			else removed.push(teamId);
		} catch (err: unknown) {
			const msg = err instanceof Error ? err.message : String(err);
