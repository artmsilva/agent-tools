...
export async function pollLeaderInbox(opts: {
	...
	let controlMessages: Awaited<ReturnType<typeof popUnreadMessages>>;
	let directMessages: Awaited<ReturnType<typeof popUnreadMessages>>;
	try {
		[controlMessages, directMessages] = await Promise.all([
			popUnreadMessages(teamDir, TEAM_CONTROL_NS, leadName),
			popUnreadMessages(teamDir, TEAM_MAILBOX_NS, leadName),
		]);
	} catch (err: unknown) {
		ctx.ui.notify(err instanceof Error ? err.message : String(err), "warning");
		return;
	}
	if (!controlMessages.length && !directMessages.length) return;
	...
	for (const m of controlMessages) {
		const approved = isShutdownApproved(m.text);
		if (approved && sanitizeName(approved.from) === sanitizeName(m.from)) {
		...
		if (rejected && sanitizeName(rejected.from) === sanitizeName(m.from)) {
		...
		if (planReq && sanitizeName(planReq.from) === sanitizeName(m.from)) {
		...
		if (peerDm && sanitizeName(peerDm.from) === sanitizeName(m.from)) {
			ctx.ui.notify(`${peerDm.from} → ${peerDm.to}: ${peerDm.summary}`, "info");
			continue;
		}

		const idle = isIdleNotification(m.text);
		if (idle && sanitizeName(idle.from) === sanitizeName(m.from)) {
		...
		ctx.ui.notify(`Ignored invalid control message from ${m.from}`, "warning");
	}

	for (const message of directMessages) {
		if (sendLeaderLlmMessage) {
			sendLeaderLlmMessage(`[Team DM] ${message.from}: ${message.text}`, { deliverAs: "followUp" });
		} else {
			ctx.ui.notify(`Message from ${message.from}: ${message.text}`, "info");
		}
	}

	// Queue batch completions at the next safe boundary. Passing followUp is
	// valid while idle too, and avoids a check-then-send race if the leader
	// starts processing between ctx.isIdle() and sendUserMessage().
	if (sendLeaderLlmMessage) {
