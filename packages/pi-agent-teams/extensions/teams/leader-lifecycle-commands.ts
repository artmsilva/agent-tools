...
import { TEAM_CONTROL_NS } from "./protocol.js";
...
type TeammateRpc = TeammateHandle;
...
export async function handleTeamCleanupCommand(opts: {
	...
	try {
		...
		const parts: string[] = [
			result.preserved ? `Preserved team recovery directory: ${teamDir}` : `Cleaned up team directory: ${teamDir}`,
		];
