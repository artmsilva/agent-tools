...
import { getDefaultWorkspaceMode } from "./spawn-policy.js";
...
export async function handleTeamSpawnCommand(opts: {
	...
	let workspaceMode: WorkspaceMode = getDefaultWorkspaceMode();
	...
	for (let i = 0; i < rest.length; i++) {
		...
		if (a === "--agent") {
			const next = rest[i + 1];
			if (!next) {
				ctx.ui.notify("Usage: /team spawn <name> [--agent <definition>]", "error");
				return;
			}
			agent = next;
			i++;
		...
		if (a === "--model") {
			const next = rest[i + 1];
			if (!next) {
				ctx.ui.notify("Usage: /team spawn <name> [fresh|branch] [shared|worktree] [plan] [--agent <definition>] [--model <provider>/<modelId>] [--thinking <level>]", "error");
				return;
			}
			model = next;
			i++;
		...
		if (a === "--thinking") {
			...
			if (!isThinkingLevel(next)) {
				ctx.ui.notify(
					`Invalid thinking level '${next}'. Valid values: off, minimal, low, medium, high, xhigh, max`,
					"error",
				);
				return;
			}
	...
	if (!nameRaw) {
		...
		if (naming.requireExplicitSpawnName) {
			ctx.ui.notify(
				"Usage: /team spawn <name> [fresh|branch] [shared|worktree] [plan] [--agent <definition>] [--model <provider>/<modelId>] [--thinking <level>]",
				"error",
			);
			return;
		}
	...
	const res = await spawnTeammate(ctx, { name: nameRaw, mode, workspaceMode, planRequired, model, thinking, agent });
	if (!res.ok) {
	...
	if (res.displayMode) extras.push(`display:${res.displayMode}`);
