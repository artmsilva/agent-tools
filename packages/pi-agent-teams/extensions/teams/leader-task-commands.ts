...
export async function handleTeamTaskCommand(opts: {
	...
	switch (taskSub) {
		...
		case "use": {
			...
			const sanitizedId = sanitizeName(newId);
			if (!sanitizedId || sanitizedId === TEAM_MAILBOX_NS || sanitizedId === TEAM_CONTROL_NS) {
				ctx.ui.notify(`Reserved or invalid task list ID: ${newId}`, "error");
				return;
			}
			setTaskListId(sanitizedId);
			await ensureTeamConfig(teamDir, { teamId, taskListId: sanitizedId, leadName, style });
			ctx.ui.notify(`Task list ID set to: ${sanitizedId}`, "info");
			await refreshTasks();
			renderWidget();
			return;
		}
