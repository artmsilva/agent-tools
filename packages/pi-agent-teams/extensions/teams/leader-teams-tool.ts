...
import { getDefaultWorkspaceMode, getMaxTeamWorkers } from "./spawn-policy.js";
...
const TeamsWorkspaceModeSchema = stringEnum(["shared", "worktree"] as const, {
	description: "Workspace isolation mode. Worktree is the safe default for source-changing teammates.",
	default: "worktree",
});

const TeamsThinkingLevelSchema = stringEnum(["off", "minimal", "low", "medium", "high", "xhigh", "max"] as const, {
	description:
...
const TeamsDelegateTaskSchema = Type.Object({
	...
	agent: Type.Optional(
		Type.String({ description: "Optional reusable agent definition from project/global `.pi/agents`." }),
	),
});
...
const TeamsToolParamsSchema = Type.Object({
	...
	agent: Type.Optional(
		Type.String({ description: "Reusable agent definition for spawned teammates." }),
	),
...
export function registerTeamsTool(opts: {
	...
	pi.registerTool({
		...
		description: [
			...
			"Optional overrides: agent='<definition>', model='<provider>/<modelId>', and thinking (off|minimal|low|medium|high|xhigh|max).",
		...
		async execute(_toolCallId, params: TeamsToolParamsType, signal, _onUpdate, ctx): Promise<AgentToolResult<unknown>> {
			...
			if (action === "member_spawn") {
				...
				const workspaceMode: WorkspaceMode = params.workspaceMode ?? getDefaultWorkspaceMode();
				...
				if (res.displayMode) lines.push(`display: ${res.displayMode}`);
			...
			if (action === "member_shutdown") {
				...
				for (const name of names) {
					...
					if (teammate) {
						setTimeout(() => {
							if (teammates.get(name) !== teammate) return;
							if (teammate.status === "stopped" || teammate.status === "error") return;
							void teammate.stop().then(
								() => setMemberStatus(teamDir, name, "offline", {
			...
			if (action === "team_done") {
				...
				for (const m of cfgWorkers) {
					...
					const ts = new Date().toISOString();
					try {
						await writeToMailbox(teamDir, TEAM_CONTROL_NS, m.name, {
							from: cfg.leadName,
							text: JSON.stringify({
								type: "shutdown_request",
			...
			const contextMode: ContextMode = params.contextMode === "branch" ? "branch" : "fresh";
			const requestedWorkspaceMode: WorkspaceMode = params.workspaceMode ?? getDefaultWorkspaceMode();
			const modelOverride = params.model?.trim();
			const spawnModel = modelOverride && modelOverride.length > 0 ? modelOverride : undefined;
			const spawnThinking = params.thinking;

			let teammateNames: string[] = [];
			const agentByTeammate = new Map<string, string>();
			const explicit = params.teammates;
			if (explicit && explicit.length) {
				teammateNames = explicit.map((n) => sanitizeName(n)).filter((n) => n.length > 0);
				if (params.agent) {
					for (const name of teammateNames) agentByTeammate.set(name, params.agent);
				}
			}

			for (const task of inputTasks) {
				if (!task.agent) continue;
				const name = sanitizeName(task.assignee ?? task.agent);
				if (!name) continue;
				const existingAgent = agentByTeammate.get(name);
				if (existingAgent && existingAgent !== task.agent) {
					return {
						content: [{ type: "text", text: `Conflicting agent definitions requested for teammate ${name}` }],
						details: { action, name, agents: [existingAgent, task.agent] },
					};
				}
				agentByTeammate.set(name, task.agent);
				if (!teammateNames.includes(name)) teammateNames.push(name);
			}
			...
			if (teammateNames.length === 0) {
				const maxTeammates = Math.max(1, Math.min(getMaxTeamWorkers(), params.maxTeammates ?? 4));
			...
			for (const name of teammateNames) {
				...
				const res = await spawnTeammate(ctx, {
					...
					agent: agentByTeammate.get(name) ?? params.agent,
				});
				if (!res.ok) {
			...
			for (const t of inputTasks) {
				...
				const agentAssignee = t.agent ? sanitizeName(t.agent) : undefined;
				const assignee = explicitAssignee ?? agentAssignee ?? teammateNames[rr++ % teammateNames.length];
				if (!assignee) {
				...
				if (!teammates.has(assignee)) {
					const res = await spawnTeammate(ctx, {
						...
						model: spawnModel,
						thinking: spawnThinking,
						agent: t.agent ?? agentByTeammate.get(assignee) ?? params.agent,
					});
					if (res.ok) {
						spawned.push(res.name);
						warnings.push(...res.warnings);
