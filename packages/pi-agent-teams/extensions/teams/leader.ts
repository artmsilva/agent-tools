...
async function createSessionForTeammate(
	...
	if (!leafId) {
		return { note: "branch(empty->fresh)", warnings };
	}

	const parentSessionFile = ctx.sessionManager.getSessionFile();
	if (!parentSessionFile) {
		return { note: "branch(in-memory->fresh)", warnings };
	}
	...
	try {
		...
		if (!branched) {
			return { note: "branch(failed->fresh)", warnings };
		}
		...
		return { note: "branch(error->fresh)", warnings };
	}
}
...
export function runLeader(pi: ExtensionAPI): void {
	...
	const startingTeammates = new Set<string>();
	...
	const spawnTeammate: SpawnTeammateFn = async (ctx, opts): Promise<SpawnTeammateResult> => {
		...
		let workspaceMode: WorkspaceMode = opts.workspaceMode ?? getDefaultWorkspaceMode();

		const name = sanitizeName(opts.name);
		if (!name) return { ok: false, error: "Missing teammate name" };
		if (teammates.has(name) || startingTeammates.has(name)) {
		...
		const teamId = currentTeamId ?? ctx.sessionManager.getSessionId();
		const teamDir = getTeamDir(teamId);
		const persisted = await loadTeamConfig(teamDir);
		const activeNames = new Set(startingTeammates);
		for (const [memberName, teammate] of teammates) {
			if (teammate.status !== "stopped" && teammate.status !== "error") activeNames.add(memberName);
		}
		for (const member of persisted?.members ?? []) {
			if (member.role === "worker" && isFreshOnlineMember(member)) activeNames.add(member.name);
		}
		const maxWorkers = getMaxTeamWorkers();
		if (activeNames.size >= maxWorkers) {
			return { ok: false, error: `Team capacity reached (${maxWorkers} active workers)` };
		}

		startingTeammates.add(name);
		let t: TeammateHandle | undefined;
		try {
			const agentDefinition = opts.agent
				? await loadTeammateAgentDefinition(opts.agent, { cwd: ctx.cwd })
				: undefined;
			const thinkingLevel = opts.thinking ?? agentDefinition?.thinking ?? pi.getThinkingLevel();
			const modelResolution = resolveTeammateModelSelection({
				modelOverride: opts.model ?? agentDefinition?.model,
		...
		const toolPolicy = resolveWorkerToolPolicy({
			activeTools: pi.getActiveTools() ?? [],
			requestedTools: agentDefinition?.tools,
			readonly: agentDefinition?.readonly,
		});
		warnings.push(...toolPolicy.warnings);
		const argsForChild: string[] = [];
		if (sessionFile) argsForChild.push("--session", sessionFile);
		argsForChild.push("--session-dir", teamSessionsDir, ...toolPolicy.args);
		...
		for (const skillPath of agentDefinition?.skillPaths ?? []) {
			argsForChild.push("--skill", skillPath);
		}
		...
		const teamInstructions = [
			`You are ${strings.memberTitle.toLowerCase()} '${name}'.`,
			`You collaborate with the ${strings.leaderTitle.toLowerCase()} through the shared task list and mailbox.`,
			workspaceMode === "worktree"
				? `Your working directory (${childCwd}) is an isolated git worktree of ${ctx.cwd}. Work only inside it; never edit the shared parent checkout.`
				: "Coordinate before editing files that another teammate may also touch.",
		].join("\n");
		const promptDir = path.join(teamDir, "prompts");
		const promptFile = path.join(promptDir, `${name}.md`);
		await fs.promises.mkdir(promptDir, { recursive: true });
		await fs.promises.writeFile(
			promptFile,
			`${[agentDefinition?.prompt, teamInstructions].filter(Boolean).join("\n\n")}\n`,
			"utf8",
		);

		const inheritedTeamEnv = Object.fromEntries(
			Object.entries(process.env).filter(
				(entry): entry is [string, string] => entry[0].startsWith("PI_TEAMS_") && typeof entry[1] === "string",
			),
		);
		const childEnv = {
			...inheritedTeamEnv,
			...
			PI_TEAMS_SPAWN_DEPTH: "1",
			PI_TEAMS_SYSTEM_PROMPT_FILE: promptFile,
			...(workspaceMode === "worktree"
				? { PI_TEAMS_WORKTREE_ROOT: childCwd, PI_TEAMS_PARENT_CWD: ctx.cwd }
				: {}),
		...
		if (!t) {
			const rpc = new TeammateRpc(name, sessionFile);
			await rpc.start({ cwd: childCwd, env: childEnv, args: argsForChild });
			t = rpc;
		}

		teammates.set(name, t);
		restoreWidget();
		const unsub = t.onEvent((event) => {
			tracker.handleEvent(name, event);
			transcriptTracker.handleEvent(name, event);
			if (event.type === "tool_execution_start" || event.type === "tool_execution_end" || event.type === "agent_end") {
				renderWidget();
			}
		});
		...
		t.onClose((code) => {
			...
			const effectiveTaskListId = taskListId ?? leaderTeamId;
			void unassignTasksForAgent(
				teamDir,
				effectiveTaskListId,
				name,
				`${formatMemberDisplayName(style, name)} ${getTeamsStrings(style).leftVerb}`,
			).finally(() => void refreshTasks().finally(renderWidget));
		...
		// Also send via control mailbox so non-RPC/manual workers can be named the same way.
		try {
		...
		await upsertMember(teamDir, {
			...
			meta: {
				...
				...(opts.agent ? { agent: opts.agent, agentPath: agentDefinition?.path } : {}),
				...(agentDefinition?.readonly ? { readonly: true } : {}),
	...
	pi.on("session_shutdown", async () => {
		...
		// Never remove teammate worktrees on session shutdown: they may contain
		// unmerged commits or dirty recovery state. Explicit cleanup and stale GC
		// remove only clean worktree checkouts and preserve their branches.
