...
export function assertValidTeamId(teamId: string): string {
	if (!/^[A-Za-z0-9][A-Za-z0-9_-]{0,127}$/.test(teamId)) {
		throw new Error(`Invalid team id: ${JSON.stringify(teamId)}`);
	}
	return teamId;
}

export function getTeamDir(teamId: string): string {
	return path.join(getTeamsRootDir(), assertValidTeamId(teamId));
}
