...
export async function claimTask(
	...
	return updated?.owner === agentName && updated.status === "in_progress" ? updated : null;
}
...
/**
 * Reset an in-progress (or otherwise non-completed) task back to pending
 * while keeping the current owner. Used when delivering a task prompt to the
 * agent fails, so the task is not stranded in_progress: it stays assigned to
 * `agentName` and will be retried on a later poll cycle instead of silently
 * disappearing.
 */
export async function requeueTaskToPending(
	teamDir: string,
	taskListId: string,
	taskId: string,
	agentName: string,
	...
	return await updateTask(teamDir, taskListId, taskId, (cur) => {
		...
		const metadata = { ...(cur.metadata ?? {}) };
		if (reason) metadata.requeuedReason = reason;
...
export async function unassignTasksForAgent(
	...
	for (const t of tasks) {
		...
		if (updated && updated.owner === undefined && updated.status === "pending") changed += 1;
	}
	return changed;
}
