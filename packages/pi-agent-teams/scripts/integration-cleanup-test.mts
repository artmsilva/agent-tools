...
console.log("\n2. cleanupWorktrees (no worktrees)");
{
	...
	assertEq(result.preservedWorktrees.length, 0, "no worktrees preserved");
...
console.log("\n3. cleanupWorktrees (with worktrees + branches)");
{
	...
	assertEq(result.removedBranches.length, 0, "branches are never deleted automatically");
	assertEq(result.preservedWorktrees.length, 0, "clean worktrees need no recovery checkout");

	// Verify checkouts are gone but recovery branches remain.
	...
	assert(branchesAfter.some((l) => l.includes(branch1)), "branch1 preserved after cleanup");
	assert(branchesAfter.some((l) => l.includes(branch2)), "branch2 preserved after cleanup");

	// Worktrees dir itself should be removed (empty)
	assert(!fs.existsSync(wtDir), "worktrees dir removed");
}

// ── Test 3b: dirty worktrees are preserved ──────────────────────
console.log("\n3b. cleanupWorktrees (dirty worktree)");
{
	const teamId = "team-dirty";
	const teamDir = path.join(teamsRoot, teamId);
	const worktreePath = path.join(teamDir, "worktrees", "worker");
	const branch = `pi-teams/${teamId}/worker`;
	fs.mkdirSync(path.dirname(worktreePath), { recursive: true });
	git(["worktree", "add", "-b", branch, worktreePath, "HEAD"], repoDir);
	fs.writeFileSync(path.join(worktreePath, "uncommitted.txt"), "recovery data");

	const result = await cleanupWorktrees({ teamDir, teamId, repoCwd: repoDir });
	assertEq(result.removedWorktrees.length, 0, "dirty worktree is not removed");
	assertEq(result.preservedWorktrees.length, 1, "dirty worktree is reported as preserved");
	assert(fs.existsSync(path.join(worktreePath, "uncommitted.txt")), "dirty file remains recoverable");
}
...
console.log("\n4. cleanupTeamDir (full cleanup including worktrees)");
{
	...
	assertEq(result.worktreeResult.removedBranches.length, 0, "branch is preserved");

	// Verify the recovery branch remains after team artifacts are removed.
	const branches = gitLines(["branch"], repoDir);
	assert(branches.some((l) => l.includes(branch)), "branch preserved in git");
}
...
// ── Test 10: cleanupWorktrees preserves unverifiable paths ──────
console.log("\n10. cleanupWorktrees (no repo context)");
{
	...
	assertEq(result.removedWorktrees.length, 0, "unverifiable path is not deleted");
	assertEq(result.preservedWorktrees.length, 1, "unverifiable path is reported for recovery");
	assert(fs.existsSync(fakePath), "fake-agent dir preserved");
}
