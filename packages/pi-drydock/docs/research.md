...
- **Network:** Apple's `--internal` is insufficient: the spike reached a host listener through the gateway. Drydock's no-network mode brings `eth0` down during a narrowly-capable bootstrap, then executes only UID 65534 commands with `NoNewPrivs: 1`; the task cannot restore the link. For online tasks, add one authenticated egress proxy/broker with DNS rebinding protection, RFC1918/link-local/metadata denial, exact host+port allowlists, audit logs, and fail-closed headless behavior. [Apple issue #1320](https://github.com/apple/container/issues/1320) [OpenAI Codex permissions](https://developers.openai.com/codex/permissions)
- **Secrets:** host Pi makes model calls. Guest receives no provider auth. Workspace transfer is Git-tracked-only, so ignored and untracked credentials stay out; intentionally committed secrets remain repository authority and cannot be detected reliably. Prefer brokered, destination-bound, short-lived credentials and never forward environment wholesale.
- **Process:** non-root task UID, read-only rootfs, tmpfs writable areas, `NoNewPrivs: 1`, empty effective/inheritable/ambient task capabilities, CPU/RAM/time limits, no virtualization or host sockets, pinned image digest. Bootstrap capabilities exist only to disable networking and drop identity.
...
The [`spike.sh`](../scripts/spike.sh) experiment proves the narrow boundary on Apple `container` 1.1.0: an unprivileged guest edits a tmpfs copy, cannot see host HOME, cannot reach the host gateway or an external IP, cannot restore networking or write its root filesystem, cannot mutate the host original, and exports a patch that applies. See the [captured run](./spike-2026-07-26.md).

Three implementation details changed through evidence: `container copy` cannot populate live tmpfs, internal networks expose the host gateway, and bootstrap capabilities must be paired with unprivileged `setpriv --nnp` task wrappers. The experimental `drydock_bash` Pi tool now uses that validated controller and returns output plus an unapplied text patch.

Next: add file-operation adapters and only then replace Pi's built-ins as one atomic mode. Large-workspace filtering, binary patch export, reviewed skill projection, and brokered egress come later. Davit remains optional last-mile UI work.

**Explicit non-goals:** running all of Pi inside the guest; mounting host `~/.pi/agent`; forwarding SSH/1Password/keychain; dynamic dotfile installation; shared `.git` mounting; push/deploy/browser/MCP support; Gondolin compatibility or shared abstractions; replacing Davit or Apple's runtime; production security claims before escape and broader host-service testing.
