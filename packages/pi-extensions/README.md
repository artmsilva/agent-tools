...
| `open-zed.ts` | `/zed` or `alt+z` opens the most recently used worktree in a new Zed window; its `⌥Z` footer item (with pi-footer) shows the target. It updates after successful `edit`, `write`, or leading `cd` shell commands and survives `/reload`. |
